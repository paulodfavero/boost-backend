...
export class CreateGainsProjectionUseCase {
  constructor(
  ...
  async execute(data: GainProjectionType): Promise<object> {
    ...
    transactions.forEach((transaction: GainProjectionRepository) => {
    ...
    transactions.map((transaction: GainProjectionRepository) => {
      const {
        bankTransactionId,
        description,
        category,
    ...
    await this.gainsProjectionRepository.createMany(dataReturn)
    return transactions
  }
}
