...
import { makeUpdateGainsProjectionUseCase } from '@/use-cases/factories/make-update-gains-projection-use-case'
...
export async function update(request: FastifyRequest, reply: FastifyReply) {
  ...
  const updateGainsProjectionUseCase = makeUpdateGainsProjectionUseCase()

  const data = await updateGainsProjectionUseCase.execute({
    organizationId,
    reqBody,
  })
